...
- **发送图片**：AI 回复中的图片 URL 或 data URL 会自动转换为图片发送（最多 10 张，单张最大 10MB）

---

## 超时补发与文件发送

配置出站 API（corpId、agentId、secret）后可启用以下功能：

- **超时补发**：如果 AI 处理时间过长导致 stream 模式超时，系统会自动通过主动消息 API 补发完整回复
- **发送文件**：AI 回复中的文件 URL（pdf、doc、xls、zip 等）会自动下载并发送给用户
