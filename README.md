...
插件支持接收用户发送的图片。当用户在企业微信中发送图片时，插件会自动下载图片并转换为 base64 格式传递给 AI，让 AI 能够"看到"并理解图片内容。

支持的消息类型：
- 单独的图片消息
- 图文混合消息（mixed）

> 💡 **提示**：图片识别功能需要 AI 模型支持多模态输入（如 Claude 3、GPT-4V 等）。
...
- Webhook 需要公网可访问（HTTP 或 HTTPS 均可）。出于安全考虑，建议只对外暴露 `/wecom` 路径
